...
/**
 * `tmdb` lists every watch-provider id that means "this platform".
 *
 * Two rules, because getting them wrong is silent — a platform with only dead
 * ids matches nothing in discover and resolves no logo, so it just quietly
 * shows up empty with a monogram instead of erroring:
 *
 *   - Include the service's own tiers (ad-supported, Essential/Premium), since
 *     they are the same catalogue behind one brand.
 *   - Exclude third-party resell channels — "… Amazon Channel", "… Roku
 *     Premium Channel", "… Apple TV channel". Those are someone else's storefront
 *     and belong to the platform a viewer would actually open.
 *
 * Retired ids stay listed after the live one: TMDB issues a new id on a rebrand
 * rather than editing the old one, and the lookups take the first id that is
 * still live, so a dead id costs nothing and records the history.
 *
